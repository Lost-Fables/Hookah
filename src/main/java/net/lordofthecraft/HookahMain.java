    ...
    private void saveHookahs() {
        ...
        for (WeakLocation loc : Hookah.getLocations()) {
            ...
            for (Integer itemPos : hookah.getIngredientSlots()) { //Ingredient Slots
                items.add(hookah.getInventory().getItem(itemPos));

                affectedSlots.add(itemPos);
            }
            if (hookah.getInventory().getItem(16) != null) { //Drug Slot
                items.add(hookah.getInventory().getItem(16));
            ...
            StringBuilder data = new StringBuilder(drugName + ";" + hookah.getCharges());
            for (Integer affectedSlot : affectedSlots) {
                data.append(";").append(affectedSlot);
            }
            hookahsFile.set("hookahs." + locationKey + ".data", data.toString());
    ...
    private void loadHookahs() {
        ...
        if (sec.getKeys(false).size() != 0) {
            for (String key : sec.getKeys(false)) {
                ...
                for (ItemStack itemData : (List<ItemStack>) hookahsFile.getList("hookahs." + key + ".items")) {
                    hookah.getInventory().setItem(slotsIter.next(), itemData);
                }
            }
        }
    }
